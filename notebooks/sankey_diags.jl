...
# ╔═╡ 2141227a-fd03-11ef-34b7-cbd73ba89d9c
using PlotlyJS

# ╔═╡ 6ddb2e6f-8f80-484a-9889-2bfe0c94045b
function create_sankey(steps::Vector{Pair{Int64, String}})
    ...
    discard_x(k) = kept_x(k) - 0.03  # associated with kept node order k (k ≥ 2)
    discard_y = 0.8
	...
	discarded = steps[end-1].first - steps[end].first
	if discarded > 0
            # Compute percentage (with two decimals)
            pct = 100 * discarded / steps[end-1].first
            label = "-$discarded (-$(round(pct, digits=2))%)"
            # Add a discard node (color red) associated with the next kept node (order = i)
            push!(node_labels, label)
            push!(node_colors, "#ff000070")
            push!(node_x, discard_x(n))
            push!(node_y, discard_y)
            discard_node_index = final_node_index
            final_node_index += 1
    ...
    # Finally, add the "Out" node (color green) as the final kept node.
    push!(node_labels, steps[end].second)
    ...
    plt = plot(trace,
		Layout(title_text = "Applied filters", font_size = 10)
	)
    return plt
end

# ╔═╡ dc059e6b-68cb-4db6-9859-c03fb260e524
let
	# steps = [
	#     100 => "Before filtering",
	#     70 => "after filter 1",
	#     50 => "after filter 2",
	#     50 => "after filter 3",
	#     20 => "after filter 4",
	#     10 => "after filter 5"
	# ]
	steps = [584382 => "All", 107235 => "In tree", 8081 => "Genome+Chromosome", 7842 => ">0 rRNA", 6955 => "completeness>95", 6840 => "contamination<5", 6783 => "tRNA>30", 6769 => "n50>50000", 6496 => "amb bases ≤ 0", 606 => "Last"]

	# steps = [x=>"s" for x in reverse(cumsum(rand(1:10, 10)))]

	p = create_sankey(steps)
...
manifest_format = "2.0"
project_hash = "5cd8df159c52f24fa31619f96f1d079b85a1ef24"
